<div class="form-group {{ $errors->has('class') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        ...
        @foreach($classes as $clas)
            <option value="{!! $clas->id !!}" @if($student->class == $clas->id) selected @endif> {!! $clas->title !!} </option>
        @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        @foreach($sections as $section)
            <option value="{{ $section->title }}" @if($student->section == $section->title) selected @endif> {!! $section->title !!} </option>
        @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        @for($i=1;$i<=70;$i++)
            <option value="{{ $i }}" @if($student->roll == $i) selected @endif> {!! $i !!} </option>
        @endfor
        </select>
    ...
    <div class="col-md-6">
        <select class="form-control" name="house" id="house" required>
            ...
            <option value="Red" @if($student->house == 'Red') selected @endif> Red </option>
            <option value="Blue" @if($student->house == 'Blue') selected @endif> Blue </option>
            <option value="Green" @if($student->house == 'Green') selected @endif> Green </option>
            <option value="Yellow" @if($student->house == 'Yellow') selected @endif> Yellow </option>
        </select>
